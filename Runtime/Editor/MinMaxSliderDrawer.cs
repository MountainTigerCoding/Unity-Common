...
// NOT ALL FIXES have been implemented from Github feed
...
internal class MinMaxSliderDrawer : PropertyDrawer
{
    ...
    private float FlexibleFloatFieldWidth(float min, float max, bool hasDecimals)
        {
            ...
            float floatFieldWidth = 14 + (Mathf.Floor(Mathf.Log10(Mathf.Abs(n)) + 1) * 8);
            if (hasDecimals) floatFieldWidth += 18;
    ...
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        float fieldWidth = ppp * (attr.DataFields && attr.FlexibleFields ?
            FlexibleFloatFieldWidth(attr.Min, attr.Max, property.propertyType == SerializedPropertyType.Vector2) :
            kFloatFieldWidth);
